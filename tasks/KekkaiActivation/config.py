...
class ActivationScheduler(Scheduler):
    ...
    success_interval: TimeDelta = Field(default=TimeDelta(hours=10), description='success_interval_help')
    failure_interval: TimeDelta = Field(default=TimeDelta(hours=10), description='failure_interval_help')

class ActivationConfig(BaseModel):
    card_rule: str = Field(default='auto', description='card_rule_help')
